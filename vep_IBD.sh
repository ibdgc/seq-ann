...
###### convert variants into vcf format ######
...
do
(
awk -F':' -v a=$i '$1==a' $1 > $i\_VarIDs.txt
...
extract_anno(){
  ...
  OUTFILE=all_$1\_vep_IBD.tsv
  OUTHEADER=all_$1\_header_vep_IBD.tsv
  ...
  cat header.txt $OUTFILE > vep_annotated$1\_IBD.tsv
...
rm all* vep* coding* annotated* header* msc*
rm ../input/sorted* ../input/coding*
conda deactivate
